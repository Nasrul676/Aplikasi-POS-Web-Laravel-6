...
<div class="">
	<div class="">
				...
				<div class="main-card card">
					<div class="card-body">
						<form class="" action="{{route('submit')}}" method="POST">
							...
							<button type="submit" class="mt-2 btn btn-primary btn-submit"><i class="fas fa-calculator"></i> Check Out</button>
						</form>
					</div>
				</div>
			</div>
		...
		<div class="row">
			<div class="col-lg-8">
				<div class="main-card card">
					<div class="card-body"><h5 class="card-title">Detail Pesanan</h5>
...
<script>
  function confirmPay(item_id) {
    ...
    swal({
        ...
        if (result.value) {
            if (bayar < total) {
                swal('Pembayaran Gagal', 'Uang Anda Kurang', 'error');
            } else {
                $('#' + item_id).submit();
            }
        } else {
